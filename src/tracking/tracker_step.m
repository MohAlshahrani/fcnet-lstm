...
function [newTargetPosition, bestScale, confidence] = tracker_step(net_x, s_x, s_z, scoreId, z_out, x_crops, pad_masks_x, targetPosition, window, p, numScales, flag)
    % run a forward pass of the CNN
    net_x.eval([z_out, {'instance', x_crops}]);
    responseMaps = reshape(net_x.vars(scoreId).value, [p.scoreSize p.scoreSize numScales]);
    % init upsampled response map
    responseMapsUP = gpuArray(single(zeros(p.scoreSize*p.responseUp, p.scoreSize*p.responseUp, numScales)));
